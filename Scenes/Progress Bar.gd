...
func set_time_text() -> void:
	...
	text.text = Globals.format_time((inst.get_playback_position() + AudioServer.get_time_since_last_mix()) / Globals.song_multiplier) + " / " + Globals.format_time(inst.stream.get_length() / Globals.song_multiplier)
