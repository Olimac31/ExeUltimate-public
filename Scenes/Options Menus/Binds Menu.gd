	...
	if !testing_binds:
		if Input.is_action_just_pressed("ui_cancel") and !selecting_key:
			if Settings.get_data("first_use") and OS.get_name() != "Android":
				...
				Settings.set_data("first_use", false)
			else:
				Scenes.switch_scene("res://Scenes/exe_title.tscn")
				if OS.get_name() == "Android":
					Settings.set_data("first_use", true)
		...
		if selecting_key:
			...
			if Input.is_action_just_pressed("ui_confirm"):
				...
				get_node("testing-text").visible = true
		...
		if Input.is_action_just_pressed("ui_confirm"):
			...
			get_node("testing-text").visible = false
...
func update_key_text():
	var i = 0
	var binds_array = Settings.get_data("binds_4")
	
	for each_label in get_node("Binds").get_children():
		each_label.text = binds_array[i]
		
		i += 1

func reset_binds_to_default():
	var default_4k_binds = ["D", "F", "J", "K"]
	
	Settings.set_data("binds_4", default_4k_binds)
	Settings.setup_binds()
	update_key_text()
...
func _on_TextureButton_pressed():
	get_node("TextureButton").release_focus()
