...
func setup_binds() -> void:
	...
	for action_num in Globals.key_count:
		...
		# Erase action and events
		# Then add the new one with the new data
		InputMap.erase_action(action)
		InputMap.add_action(action)
		InputMap.action_add_event(action, new_Event)
		
		# Arrow keys as default/alt
		var arrow_event = InputEventKey.new()
		match(action):
			"gameplay_0":
				arrow_event.set_scancode(16777231)
			"gameplay_1":
				arrow_event.set_scancode(16777234)
			"gameplay_2":
				arrow_event.set_scancode(16777232)
			"gameplay_3":
				arrow_event.set_scancode(16777233)
